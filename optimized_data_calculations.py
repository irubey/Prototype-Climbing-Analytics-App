...
def perform_calculations(first_input):
    ...
    def rename_and_clean_ticks(df):
        ...
        df.drop(['user_grade', 'notes', 'route_stars', 'user_stars'], axis=1, inplace=True)

        # Change data types
        df['route_name'] = df['route_name'].astype(str)
        df['route_grade'] = df['route_grade'].astype(str)
        df['pitches'] = df['pitches'].astype('int8')
        df['location'] = df['location'].astype(str)
        df['style'] = df['style'].astype('category')
        df['lead_style'] = df['lead_style'].astype('category')
        df['route_type'] = df['route_type'].astype('category')
        df['binned_code'] = df['binned_code'].astype('int16')
        return df

    def populate_discipline(user_ticks):
        #Add discipline
        conditions = [
        ...
        user_ticks.drop('route_type', axis=1, inplace=True)

        return user_ticks

    def populate_username(df,usercsvlink):
        #get username
    ...
    def populate_binned_codes(user_ticks,binned_code_dict):
        ...
        user_ticks['binned_code'] = user_ticks['binned_code'].astype('int16')
        return user_ticks

    def populate_send_bool(user_ticks):
        ...
        user_ticks['send_bool'] = user_ticks['send_bool'].astype(bool)

        user_ticks.drop('style', axis=1, inplace=True)

        return user_ticks

    def populate_binned_grades(user_ticks, binned_code_dict):
        ...
        user_ticks['binned_grade'] = user_ticks['binned_grade'].astype('category')

        return user_ticks

    def populate_length_categories(user_ticks):
    ...
    def populate_locations(user_ticks):
        ...
        user_ticks['location'] = user_ticks['location'].astype('category')

        return user_ticks
    
    def populate_cur_max_sends(user_ticks):
        ...
        user_ticks['cur_max_rp_sport'] = user_ticks['cur_max_rp_sport'].astype('int16')
        user_ticks['cur_max_rp_trad'] = user_ticks['cur_max_rp_trad'].astype('int16')
        user_ticks['cur_max_boulder'] = user_ticks['cur_max_boulder'].astype('int16')
        
        return user_ticks
        
    def populate_difficulty_categories(user_ticks):
    ...
    def populate_season_category(user_ticks):
        ...
        user_ticks.drop(['tick_month', 'tick_year'], axis=1, inplace=True)
    ...
    def create_df_ticklist(usercsvlink):
        # Step 1: Download user ticks
        user_ticks = download_user_ticks(usercsvlink)

        # Step 2: Rename and Clean
        user_ticks = rename_and_clean_ticks(user_ticks)

        # Step 3: Populate new columns
        user_ticks = populate_discipline(user_ticks)
        user_ticks = populate_send_bool(user_ticks)
        user_ticks, username = populate_username(user_ticks, usercsvlink)
        user_ticks = populate_binned_codes(user_ticks, binned_code_dict)
        user_ticks = populate_binned_grades(user_ticks, binned_code_dict)
        user_ticks = populate_length_categories(user_ticks)
        user_ticks = populate_locations(user_ticks)
        user_ticks = populate_cur_max_sends(user_ticks)
        user_ticks = populate_difficulty_categories(user_ticks)
        user_ticks = populate_season_category(user_ticks)


        return user_ticks, username
    
    user_ticks, username = create_df_ticklist(usercsvlink)

    def pyramid1(user_ticks):
        ...
        columns_to_drop = ['cur_max_rp_sport','cur_max_rp_trad','cur_max_boulder','send_bool','difficulty_category']
    ...
    sport_pyramid, trad_pyramid, boulder_pyramid = pyramid1(user_ticks)
