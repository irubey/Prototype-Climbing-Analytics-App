// Move getGradeForCode outside DOMContentLoaded and make it accept binnedCodeDict
function getGradeForCode(binnedCode, binnedCodeDict) {
...
document.addEventListener("DOMContentLoaded", function () {
  function characterVizChart(targetId, inputData, binnedCodeDict) {
    ...
    // Get unique binned codes for the legend first
    ...
    // Process data and check for missing characteristics
...
function processData(data, keys) {
  ...
  const energyData = data.some((d) => d.route_characteristic)
    ? processCategory(
        data,
  ...
  const lengthData = processCategory(
    data,
    "length_category",
    ["short", "medium", "long", "multipitch"],
    keys
  );

  const angleData = data.some((d) => d.route_style)
    ? processCategory(
        data,
        "route_style",
  ...
  return [
    {
      group: "Energy System",
      data: energyData,
      hasMissingData: !data.some((d) => d.route_characteristic),
    },
    { group: "Length", data: lengthData, hasMissingData: false },
    {
      group: "Angle",
      data: angleData,
      hasMissingData: !data.some((d) => d.route_style),
    },
  ];
}

function processCategory(data, attribute, validValues, keys) {
...
function showTooltip(
  ...
  let tooltipContent = `
    <div style="margin-bottom: 5px;">
      <strong>${getGradeForCode(gradeKey, binnedCodeDict)} ${
    data.category
  }</strong>
      ${
        isPinned
  ...
  dataset.forEach((group, groupIndex) => {
    if (group.hasMissingData) {
      ...
      const messageG = g
        ...
        .attr(
          "transform",
          `translate(${groupIndex * groupWidth + groupPadding / 2}, ${
            height / 2 - 20
          })`
        );

      // Add edit button as a clickable rect with text
      const buttonG = messageG
        .append("g")
        ...
        .on("click", () => {
          ...
          // Redirect to pyramid-input with username
          window.location.href = `/pyramid-input?username=${username}`;
        });
