...
function progressionDifficultyChart(userTicksData, targetId) {
  ...
  function getColor(category, type) {
    const colors = {
      difficulty: {
        Project: "#FF4B4B", // Bright red
        "Tier 2": "#2ECC71", // Emerald green
        "Tier 3": "#F1C40F", // Sunflower yellow
        "Tier 4": "#9B59B6", // Amethyst purple
        "Base Volume": "#3498DB", // Peter river blue
      },
    };

    return colors[type][category] || "#95A5A6"; // Default to a neutral gray
  }
  ...
  function prepareData(userTicksData, mapFn) {
    ...
    userTicksData.forEach((d) => {
      const category = d.difficulty_category;
    ...
    return userTicksData.map((d) => ({
      ...
      pitches: d.pitches || 0,
      categoryStats: categoryStats[d.difficulty_category],
    }));
  }
  ...
  function createChart(
    ...
    // Y-axis label with adjusted positioning
    svg
      .append("text")
      .attr("class", "y-label")
      .attr("text-anchor", "middle")
    ...
    const tooltip = d3
      .select("body")
      .selectAll(".difficulty-tooltip")
      .data([0])
      .join("div")
      .attr("class", "difficulty-tooltip")
      .style("opacity", 0)
    ...
    categories.forEach((cat) => {
      svg
        ...
        .on("mouseover", function (event) {
          const lastPoint = dataByCategory[cat][dataByCategory[cat].length - 1];
  ...
  var discipline = d3
    .select("input[name='diff-cat-discipline-filter']:checked")
    .node().value;
  var timeFrame = d3
    .select("input[name='diff-cat-time-filter']:checked")
    .node().value;
  ...
  createChart(
    ...
    "category",
    "difficulty",
    (cat) => getColor(cat, "difficulty"),
    (d) => true,
    "#diff-cat"
  );
}

// Add event listener for DOM ready if needed
