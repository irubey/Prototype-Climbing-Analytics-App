document.addEventListener("DOMContentLoaded", function() {
  ...
  function daysOutsideChart(targetId, userTicksData){
    ...
    function prepareData(userTicksData) {
      ...
      return userTicksData.reduce((acc, curr) => {
        ...
        if (!isNaN(date) && seasonCategory && seasonCategory.trim() && discipline && discipline.trim()) {
    ...
    function groupDataByCategory(data) {
      return data.reduce((groups, item) => {
        ...
        groups[seasonKey][item.discipline] = (groups[seasonKey][item.discipline] || 0) + 1;
        return groups;
      }, {});
    }

    function extractUniqueDisciplines(userTicksData) {
      return Object.keys(userTicksData.reduce((unique, item) => {
    ...
    function createBarChartDaysOutside(svg, histogramData, sortedDisciplines, x, y, color) {
    ...
    function daysOutside(userTicksData, targetId, customSVG = SVG_DEFAULTS) {
      d3.select(targetId).select("svg").remove();

      var timeFrame = d3.select("input[name='days-outside-time-filter']:checked").node().value;
      
      // Apply filters
      var filteredData = CommonFilters.filterByTime(userTicksData, timeFrame);
      ...
      createLegend(svg, sortedDisciplines, color, width - margin.left - margin.right);


    }
    
    daysOutside(userTicksData, targetId, customSVG = SVG_DEFAULTS);
  }

  d3.selectAll("input[name='days-outside-time-filter']").on('change', function(){
    daysOutsideChart('#days-outside', userTicksData, customSVG=SVG_DEFAULTS);
 });
