...
from app.models.enums import ClimbingDiscipline, LogbookType, CruxAngle, CruxEnergyType
...
async def test_update_pyramid_existing(mock_db):
    ...
    mock_tick = MagicMock(spec=False)
    mock_pyramid = MagicMock(spec=False)
    ...
    # Create a dictionary to store attributes set on the mock_pyramid
    pyramid_attrs = {}
    
    # Use a patched setattr that intercepts calls to our mock_pyramid
    def side_effect_setattr(obj, name, value):
        # Store attribute values when set on our mock_pyramid
        if obj is mock_pyramid:
            pyramid_attrs[name] = value
        # Call the real setattr for other objects
        else:
            original_setattr(obj, name, value)
    
    # Save the original setattr function
    original_setattr = setattr
    
    # Patch both setattr
    with patch('app.api.v1.endpoints.data.setattr', side_effect=side_effect_setattr):
        # Act
        ...
        # Verify attributes were set with correct values
        for attr_name, expected_value in [
            ...
            assert attr_name in pyramid_attrs, f"Attribute {attr_name} was never set"
            assert pyramid_attrs[attr_name] == expected_value, f"Expected {attr_name}={expected_value}, got {pyramid_attrs[attr_name]}"
